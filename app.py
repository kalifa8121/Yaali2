...
def ceo_mudaraba_list():
    ...
    for c in mudaraba_custs:
        ...
        <tr style="border-bottom:1px solid #e2e8f0; font-size:12px;">
            ...
            <td style="padding:8px; color:#6b21a8;">+{cust_profit:,.2f} Birr</td>
            <td style="padding:8px; color:#047857;">+{ceo_profit:,.2f} Birr</td>
        </tr>
        """
        ...
        <table style="width:100%; border-collapse:collapse; text-align:left;">
            <thead>
                <tr style="background:#f8fafc; font-size:11px; color:#64748b; border-bottom:1px solid #e2e8f0;">
                    ...
                    <th style="padding:8px;">Maqaa</th>
                    <th style="padding:8px;">Balance</th>
                    <th style="padding:8px;">Qooda Maammila (50%)</th>
                    <th style="padding:8px;">Qooda CEO (50%)</th>
                </tr>
            </thead>
            <tbody>
                {rows_html if rows_html else '<tr><td colspan="5" style="padding:16px; text-align:center;">Maammilli Mudarabaa hin jiru.</td></tr>'}
            </tbody>
        </table>
    </div>
    """
...
def register():
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size: 16px; margin-bottom: 12px; color:#065f46;">⚡ Galmee Maammilaa (Maker T24)</h2>
        {f"<p style='background:#dcfce7; color:#166534; padding:10px; border-radius:6px; font-size:12px; font-weight:bold; margin-bottom:12px;'>{msg}</p>" if msg else ""}
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
...
def edit_customer(cust_id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute("SELECT * FROM customers WHERE customer_id = ?", (cust_id,))
        customer = cursor.fetchone()
        msg = f"✅ Odeeffannoon maammilaa milkaa'inaan foyya'eera!"
    ...
    <div class="box">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>📸 Suuraa Fuula Jijjiiri</label>
            ...
            <div class="form-group">
                <label>🆔 ID / Fayda Jijjiiri</label>
                <input type="file" name="national_id" accept="image/*,.pdf" class="input-field">
            </div>
            <button type="submit" class="btn-submit" style="background:#2563eb;">💾 Foyya'e Save Godhi</button>
        </form>
    </div>
    """
    return render_template_string(HTML_LAYOUT.replace("{% block content %}{% endblock %}", content), notifications=NOTIFICATIONS)

# --- RULE 10: CEO ONLY FREEZE/UNFREEZE WITH REASON & STATEMENT/FORM PRINTS ---
@app.route('/ceo_freeze_customer/<cust_id>', methods=['GET', 'POST'])
def ceo_freeze_customer(cust_id):
    if 'role' not in session or session['role'] != 'CEO':
        return "🚫 CEO Qofa!", 403
    ...
    if request.method == 'POST':
        action = request.form.get('action')
        reason = request.form.get('freeze_reason', 'Akkakuu koodii')
        if action == 'freeze':
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#581c87;">🔒 Freeze / Unfreeze Maammila: {c['full_name']} ({cust_id})</h2>
        <p style="font-size:12px; margin:8px 0;">Status Ammaa: <b>{c['freeze_status']}</b></p>
        <form method="POST">
            <input type="hidden" name="action" value="{'unfreeze' if c['freeze_status']=='FROZEN' else 'freeze'}">
            {f'<div class="form-group"><label>Sababa Ugguraa (Reason)</label><input type="text" name="freeze_reason" required class="input-field"></div>' if c['freeze_status']!='FROZEN' else ''}
            <button type="submit" class="btn-submit" style="background:{'#16a34a' if c['freeze_status']=='FROZEN' else '#dc2626'};">
                {'🔓 Unfreeze Maammila Kana' : '🔒 Freeze Maammila Kana (Sababa wajjin)'}
            </button>
        </form>
    </div>
    """
...
def islamic_loan():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        msg = f"📜 Liqaa Islaamaa {financing_type} ({principal:,.2f} Birr) uumameera!"

    cursor.execute("SELECT * FROM islamic_financing ORDER BY timestamp DESC")
    loans_list = cursor.fetchall()
    conn.close()

    options_html = "".join([f'<option value="{c["customer_id"]}">{c["full_name"]} ({c["customer_id"]})</option>' for c in active_customers])
    loans_html = ""
    for l in loans_list:
        approval_actions = ""
        ...
        <div class="item-card" style="border-left: 4px solid #16a34a;">
            <div style="font-size:12px; font-weight:bold; color:#16a34a;">{l['loan_id']} ({l['financing_type']})</div>
            <div style="font-size:13px; font-weight:bold; margin-top:2px;">Maammila: {l['customer_name']}</div>
            <div style="font-size:11px; color:#475569;">Kaabitaala: {l['principal_amount']:,.2f} Birr | Waliigala Deebi'u: {l['total_repayment']:,.2f} Birr</div>
            <div style="margin-top:6px;">{approval_actions}</div>
        </div>
        """
    ...
    <div class="box">
        <form method="POST">
            <div class="form-group"><label>Maammila</label><select name="customer_id" required class="input-field">{options_html}</select></div>
            ...
            <div class="form-group"><label>Hamma Kaabitaala</label><input type="number" step="0.01" name="principal_amount" required class="input-field"></div>
            ...
            <div class="form-group"><label>Tenure Months</label><input type="number" name="tenure_months" value="12" required class="input-field"></div>
...
# --- RULE 11: MANAGER & AUDITOR TRANSACTION APPROVAL WITH SIGNATURE VIEW ---
@app.route('/pending')
def pending():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT customer_id, full_name, phone, gender, account_type, photo_path, signature_path FROM customers WHERE status='PENDING_APPROVAL'")
    pending_custs = cursor.fetchall()

    cursor.execute("""
        SELECT t.*, c.photo_path, c.signature_path, c.phone, c.freeze_status, c.freeze_reason 
        FROM transactions t LEFT JOIN customers c ON t.customer_id = c.customer_id 
        WHERE t.status = 'PENDING_MANAGER' ORDER BY t.timestamp DESC
    """)
    ...
    for c in pending_custs:
        ...
        <div class="item-card" style="background:#eff6ff;">
            <div style="font-size:12px; font-weight:bold;">Acc: {c['customer_id']} - {c['full_name']}</div>
            <div class="img-grid">
    ...
    for r in pending_txns:
        cards_html += f"""
        <div class="item-card">
            <div style="font-size:12px; font-weight:bold;">Ref: {r['ft_reference']} | {r['txn_type']} ({r['amount']:,.2f} Birr)</div>
            <div style="font-size:11px; color:#64748b; margin:4px 0;">Maammila: {r['customer_name']}</div>
            <div class="img-grid" style="max-width:200px;">
                <img src="/uploads/{r['signature_path']}" title="Mallattoo Maammilaa (Anti-Fraud)">
            </div>
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size: 16px; color:#b45309; margin-bottom: 8px;">📋 Approval Eeggachaa Jiran (Manager & Auditor)</h2>
        {cards_html if cards_html else '<p style="text-align:center; color:#64748b;">Wanti mirkanaa\'u hin jiru.</p>'}
    </div>
    """
    return render_template_string(HTML_LAYOUT.replace("{% block content %}{% endblock %}", content), notifications=NOTIFICATIONS)
...
def manager_action(action, txn_id):
    ...
    if t:
        if action == 'approve':
            ...
            comm = t['commission']
            if t['txn_type'] == 'DEPOSIT':
                cursor.execute("UPDATE customers SET balance = balance + ? WHERE customer_id = ?", (amt, t['customer_id']))
            elif t['txn_type'] == 'WITHDRAWAL':
                cursor.execute("UPDATE customers SET balance = balance - ? WHERE customer_id = ?", (amt + comm, t['customer_id']))
            elif t['txn_type'] == 'T24_TRANSFER':
                cursor.execute("UPDATE customers SET balance = balance - ? WHERE customer_id = ?", (amt, t['customer_id']))
                if t['target_account']:
                    cursor.execute("UPDATE customers SET balance = balance + ? WHERE customer_id = ?", (amt, t['target_account']))
        else:
