...
# --- NEON / RENDER POSTGRESQL CONNECTION SUPPORT ---
...
# --- RULE 12 & 13 & 14: EXACT COMMISSION RULES ---
...
def get_transfer_commission(amount):
    return amount * 0.02 # Rule 13: 2%

def get_print_commission(amount):
    return amount * 0.01 # Rule 14: 1%
...
def init_db():
    ...
    serial_pk = "SERIAL PRIMARY KEY" if is_postgres else "TEXT PRIMARY KEY"
...
def dashboard():
    ...
    if role in ['LOAN_OFFICER', 'CEO', 'MANAGER']:
        loan_btn = """
        <a href="/islamic_loan" class="btn-card"><span class="icon">📜</span><span>Mudaraba & Murabaha Loan</span></a>
        """
    ...
    if role == 'CEO':
        ...
        """
        # --- SUB-INSTRUCTION 1: CAPITAL WALIGALAA CEO QOFTATTI AKKA MULLATU GODHI ---
        net_capital_html = f"""
        <div class="card-net">
        ...
        <a href="/external_agents" class="btn-card btn-card-ceo"><span class="icon">🌐</span><span>External Bank Agents</span></a>
...
# --- FEATURE 1: AUDITOR & MANAGER TRANSACTION ID RECEIPT PRINT ---
...
def print_receipt_search():
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#065f46; margin-bottom:8px;">🖨️ Transaction ID Fayyadamuun Nagahee Maxxansi</h2>
        <p style="font-size:11px; color:#64748b; margin-bottom:14px;">Maker, Auditor ykn Manager transaction ID galchuun nagahee maxxansuu danda'u.</p>
...
# --- FEATURE 2: LISTII MAAMMILTOOTAA (CUSTOMERS LIST) ---
@app.route('/customers')
def customers_list():
    if 'role' not in session:
        return redirect('/login')
    ...
    for c in custs:
        ...
        actions = f"""
        <a href="/statement/{c['customer_id']}" class="btn-action btn-blue" style="padding:4px 8px; font-size:10px;">Statement</a>
        """
        if session['role'] == 'MANAGER':
            actions += f""" <a href="/edit_customer/{c['customer_id']}" class="btn-action btn-orange" style="padding:4px 8px; font-size:10px;">Edit</a>"""
        if session['role'] == 'CEO':
            actions += f""" <a href="/ceo_freeze/{c['customer_id']}" class="btn-action {"btn-green" if c['freeze_status']=="FROZEN" else "btn-red"}" style="padding:4px 8px; font-size:10px;">{"Unfreeze" if c['freeze_status']=="FROZEN" else "Freeze"}</a>"""

        rows_html += f"""
        <tr style="border-bottom:1px solid #e2e8f0; font-size:11px;">
            <td style="padding:8px; font-weight:bold;">{c['customer_id']}</td>
            <td style="padding:8px;">{c['full_name']}<br><span style="color:#64748b;">{c['phone']} ({c['gender']})</span></td>
    ...
    <div class="box">
        ...
        <div style="overflow-x:auto;">
            <table style="width:100%; border-collapse:collapse; text-align:left;">
                ...
                <tbody>
                    {rows_html if rows_html else '<tr><td colspan="5" style="padding:16px; text-align:center; color:#64748b;">Maammilli galmaa\'e hin jiru.</td></tr>'}
                </tbody>
            </table>
        </div>
    </div>
...
# --- FEATURE 3: REVERSAL REQUEST & WORKFLOW (AUDITOR -> MANAGER -> CEO) ---
...
def auditor_reversal_request():
    ...
    if request.method == 'POST':
        ...
        if not txn:
            msg = "❌ Transaction ID / FT Ref kanaan hin argamne!"
        else:
            ...
            conn.commit()
            msg = f"⚠️ Gaaffiin Reversal milkaa'inaan ergameera (ID: {rev_id}). Manager approval eegaa jira."
            add_notification(f"Auditor reversal gaafateera txn: {txn['txn_id']}")
        conn.close()

    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#c2410c; margin-bottom:8px;">⚠️ Transaction Reversal Gaafachuu (Auditor)</h2>
        <p style="font-size:11px; color:#64748b; margin-bottom:12px;">Auditor transaction ID galchee sababa waliin gaaffii reversal Manager fi CEO biratti dabarsa.</p>
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" name="txn_id" required class="input-field" placeholder="Fkn: TXN-1724200000">
            </div>
            <div class="form-group">
                <label>Sababa Reversal (Reason)</label>
                <textarea name="reason" required class="input-field" rows="3" placeholder="Sababa bal'aa barreessi..."></textarea>
            </div>
...
def reversals_list():
    ...
    for r in revs:
        ...
        if session['role'] == 'MANAGER' and r['manager_approved'] == 0:
            ...
            action_btn = f"<b>{r['status']}</b>"

        rows_html += f"""
        <div class="item-card">
            <div style="font-size:12px; font-weight:bold; color:#7c3aed;">Reversal ID: {r['reversal_id']} | Txn: {r['txn_id']}</div>
            <div style="font-size:12px; margin-top:4px;"><b>Sababa:</b> {r['reason']}</div>
            <div style="font-size:11px; color:#64748b; margin-top:2px;">Gaafate: {r['requested_by']} | {r['timestamp']}</div>
            <div style="margin-top:8px; display:flex; gap:6px;">{action_btn}</div>
        </div>
        """

    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#7c3aed; margin-bottom:12px;">🔄 Bulchiinsa Reversal Transactions</h2>
        {rows_html if rows_html else '<p style="text-align:center; padding:20px; color:#64748b; font-size:12px;">Gaaffiin reversal hin jiru.</p>'}
    </div>
    """
    return render_template_string(HTML_LAYOUT.replace("{% block content %}{% endblock %}", content), notifications=NOTIFICATIONS)
...
def approve_reversal(level, rev_id):
    ...
    if level == 'manager' and session['role'] == 'MANAGER':
        ...
        # Perform actual database transaction reversal on customer balance
        ...
        if txn:
            ...
            if t_type == 'DEPOSIT':
                cursor.execute("UPDATE customers SET balance = balance - ? WHERE customer_id = ?", (amt, c_id))
            elif t_type == 'WITHDRAWAL':
                cursor.execute("UPDATE customers SET balance = balance + ? WHERE customer_id = ?", (amt + comm, c_id))
...
# --- FEATURE 4: CEO & MANAGER TRANSACTION FILTER ---
...
def ceo_commission():
    ...
    query = "SELECT txn_id, ft_reference, customer_name, amount, commission, txn_type, timestamp FROM transactions WHERE status='APPROVED'"
    params = []
    ...
    rows_html = ""
    for t in txns:
        # --- SUB-INSTRUCTION 2: COMMISION MURAMU NAGAHEE FI STATEMENT IRRATTI AKKA HIN MULLANE GODHI ---
        # (Commission is stored in db but omitted in display as requested)
        rows_html += f"""
    ...
    content = f"""
    <div class="card-ceo-profit">
        <div class="net-title">📊 Filter Transactions (CEO & Manager)</div>
        <div class="net-amount">{total_amt:,.2f} Birr</div>
        <p style="font-size:11px; opacity:0.9; margin-top:4px;">Waliigala Hamma Transaction Filterame</p>
    </div>

    <div class="box">
        <h3 style="font-size:13px; margin-bottom:8px; color:#475569;">🔍 Filter Guyyaa & Gosa Transaction</h3>
        <form method="GET" style="display:flex; flex-direction:column; gap:8px;">
            <div style="display:flex; gap:8px;">
                <div style="flex:1;">
                    <label style="font-size:11px; font-weight:bold;">Gosa Txn</label>
                    <select name="filter_type" class="input-field">
                        <option value="">Hunda (All)</option>
                        ...
                        <option value="T24_TRANSFER" {'selected' if filter_type=='T24_TRANSFER' else ''}>T24_TRANSFER</option>
                    </select>
                </div>
                <div style="flex:1;">
            ...
            <button type="submit" class="btn-action btn-purple" style="padding:10px; text-align:center;">Filter Gochi</button>
        </form>
    </div>
    ...
    <div class="box" style="padding:0; overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; text-align:left;">
            ...
            <tbody>
                {rows_html if rows_html else '<tr><td colspan="5" style="padding:16px; text-align:center; color:#64748b;">Transaction-ni argame hin jiru.</td></tr>'}
            </tbody>
        </table>
    </div>
    """
...
# --- FEATURE 5: USER MANAGEMENT (MANAGER ROLE/PWD & CEO CREATE/EDIT/RESET) ---
...
# --- FEATURE 6: MUDARABA PRIVATE LIST & REGISTRATION SECTION ---
@app.route('/ceo_mudaraba_list')
def ceo_mudaraba_list():
    if 'role' not in session or session['role'] != 'CEO':
        return "🚫 CEO Qofatu Listii Mudarabaa Ilaaluu Danda'a!", 403
    ...
    content = f"""
    <div class="card-ceo-profit">
        <div class="net-title">🔒 MUDARABA PRIVATE INVESTMENT LIST</div>
        <div class="net-amount">{total_mudaraba_bal:,.2f} Birr</div>
        <p style="font-size:11px; opacity:0.9; margin-top:4px;">Kuusaa Waliigala Maammiltoota Mudaraba</p>
    </div>

    <h3 style="font-size:14px; margin-bottom:8px; color:#334155;">📋 Tarree Mudaraba Private (50/50 Profit Split)</h3>
    <div class="box" style="padding:0; overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; text-align:left;">
            <thead>
                <tr style="background:#f8fafc; font-size:11px; color:#64748b; border-bottom:1px solid #e2e8f0;">
...
# --- FEATURE 7: EXTERNAL BANK AGENTS (10% COMMISSION SHARE) ---
...
def external_agents():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS external_agents (
            ...
            agent_name TEXT,
            phone TEXT,
            total_transactions REAL DEFAULT 0.0,
    ...
    msg = None
    if request.method == 'POST':
        aname = request.form.get('agent_name').strip()
        aphone = request.form.get('phone').strip()
        agent_id = f"EXT-AGT-{int(datetime.datetime.now().timestamp())}"
        cursor.execute("INSERT INTO external_agents (agent_id, agent_name, phone, status) VALUES (?, ?, ?, 'ACTIVE')", (agent_id, aname, aphone))
        conn.commit()
        msg = f"✅ External Agent haaraan ({aname}) milkaa'inaan galmaa'eera!"

    cursor.execute("SELECT * FROM external_agents")
    agents = cursor.fetchall()
    conn.close()

    rows_html = ""
    for a in agents:
        agent_comm = a['commission_earned'] * 0.10 # 10% commission rule
        rows_html += f"""
        <tr style="border-bottom:1px solid #e2e8f0; font-size:12px;">
            <td style="padding:8px; font-weight:bold;">{a['agent_id']}</td>
            <td style="padding:8px;">{a['agent_name']}</td>
            <td style="padding:8px;">{a['phone']}</td>
            <td style="padding:8px;">{a['total_transactions']:,.2f} Birr</td>
            <td style="padding:8px; font-weight:bold; color:#047857;">+{agent_comm:,.2f} Birr (10%)</td>
        </tr>
        """

    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#581c87; margin-bottom:8px;">🌐 External Bank Agents Bulchiinsa (10% Commission)</h2>
        <p style="font-size:11px; color:#64748b; margin-bottom:12px;">Maammila alaa uumanii fi transaction raawwatan hogganaa.</p>
        {f"<p style='background:#dcfce7; color:#166534; padding:10px; border-radius:6px; font-size:12px; font-weight:bold; margin-bottom:12px;'>{msg}</p>" if msg else ""}
        <form method="POST">
            <div class="form-group">
                <label>Maqaa Agentii Alaa</label>
                <input type="text" name="agent_name" required class="input-field" placeholder="Maqaa...">
            </div>
            <div class="form-group">
                <label>Lakkoofsa Bilbilaa</label>
                <input type="text" name="phone" required class="input-field" placeholder="09...">
            </div>
        ...
        <table style="width:100%; border-collapse:collapse; text-align:left;">
            <thead>
                <tr style="background:#f8fafc; font-size:11px; color:#64748b; border-bottom:1px solid #e2e8f0;">
                    ...
                    <th style="padding:8px;">Maqaa</th>
                    <th style="padding:8px;">Bilbila</th>
                    <th style="padding:8px;">Waliigala Txn</th>
                    <th style="padding:8px;">Qoodda 10% Commission</th>
                </tr>
            </thead>
            <tbody>
                {rows_html if rows_html else '<tr><td colspan="5" style="padding:16px; text-align:center; color:#64748b;">Agentiin alaa galmaa\'e hin jiru.</td></tr>'}
            </tbody>
        </table>
    </div>
    """
...
# --- FEATURE 8: MAKER TRANSACTION WITH ANTI-FRAUD VIEW & VERIFY ---
...
def transaction():
    ...
    if request.method == 'POST':
        ...
        if amount != confirm_amount:
            ...
            # Calculate correct commission based on rules 12 & 13
            if txn_type == 'WITHDRAWAL':
                commission = get_withdrawal_commission(amount)
            elif txn_type == 'T24_TRANSFER':
                commission = get_transfer_commission(amount)
            else:
                commission = 0.0

            total_req = amount + commission if txn_type == 'WITHDRAWAL' else amount
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size: 16px; color:#065f46; margin-bottom: 12px;">💸 Transaction Raawwadhu (Maker Anti-Fraud Check)</h2>
        {f"<p style='background:{'#dcfce7' if msg_type=='green' else '#fee2e2'}; color:{'#166534' if msg_type=='green' else '#991b1b'}; padding:10px; border-radius:6px; font-size:12px; font-weight:bold; margin-bottom:12px;'>{msg}</p>" if msg else ""}
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <select name="txn_type" id="txn_type" class="input-field" onchange="toggleTargetAcc()">
                    ...
                    <option value="T24_TRANSFER">🔄 T24 Account Transfer (Akaawuntii irraa)</option>
                </select>
            </div>
            ...
            <div id="verification_box" style="display:none; background:#f0fdf4; border:1px solid #bbf7d0; padding:12px; border-radius:8px; margin-bottom:12px;">
                <h4 style="font-size:11px; color:#166534; margin-bottom:6px;">🛡️ MIRKANEESSA MAAMMILAA (ANTI-FRAUD VERIFY)</h4>
                ...
                <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #bbf7d0;">
                    <label style="font-size:12px; font-weight:bold; color:#166534; display:flex; align-items:center; gap:6px;">
                        <input type="checkbox" required name="verify_check"> Ani suuraa fi mallattoo maammilaa sirriitti ilaalee mirkaneesseera.
                    </label>
                </div>
            </div>

            <div class="form-group" id="target_acc_group" style="display:none;">
                <label>Account ID Nama Fudhatuu (Verify Target Account)</label>
    ...
    function fetchCustomerVerification() {{
        ...
        .then(data => {{
            if(data.success) {{
                ...
                stElem.innerHTML = (data.freeze_status === 'FROZEN') ? '<b style="color:red;">🔒 FROZEN</b>' : '<b style="color:green;">✅ ACTIVE</b>';
...
def maker_receipts():
    ...
    cursor.execute("""
        ...
        FROM transactions WHERE created_by = ? ORDER BY timestamp DESC
    """, (session['username'],))
    txns = cursor.fetchall()
    conn.close()
...
# --- FEATURE 9: MANAGER CUSTOMER EDIT (NAME, AGE, GENDER, PHOTO, SIGNATURE, ID) ---
...
def edit_customer(cust_id):
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size: 16px; color:#2563eb; margin-bottom: 4px;">✏️ Odeeffannoo Maammilaa Foyyeessi (Manager Edit)</h2>
...
# --- FEATURE 10: CEO FREEZE/UNFREEZE WITH REASON & BLANK FORM PRINT ---
@app.route('/ceo_freeze/<cust_id>')
def ceo_freeze(cust_id):
    if 'role' not in session or session['role'] != 'CEO':
        return "🚫 Hayyama CEO Qofa!", 403
    ...
    if c:
        ...
        reason = 'Sababa CEO tiin uggurame' if new_status == 'FROZEN' else ''
...
def ceo_blank_form():
    ...
    <!DOCTYPE html>
    <html>
    <head><title>Formii Duwwaa - Imana Microfinance</title></head>
    <body style="font-family:sans-serif; padding:30px; max-width:800px; margin:0 auto;">
        <h2 style="text-align:center; color:#065f46;">IMANA FREE INTEREST MICROFINANCE</h2>
        <p style="text-align:center; font-size:12px;">FORMII GALMEE MAAMMILAA FI KAFFAALTII DUWWAA</p>
        <hr style="margin:20px 0;">
        <p><b>Maqaa Guutuu:</b> _________________________________________________</p>
        <p><b>Lakkoofsa Bilbilaa:</b> ___________________________________________</p>
        <p><b>Gosa Akkaawuntii:</b> [  ] Wadia Savings   [  ] Mudaraba Investment</p>
        <p><b>Hamma Maallaqaa:</b> _____________________________________________</p>
        <br><br>
        <div style="display:flex; justify-content:space-between; margin-top:50px;">
            <div>_______________________<br>Mallattoo Maammilaa</div>
            <div>_______________________<br>Mallattoo Hojjataa</div>
        </div>
...
# --- FEATURE 11: MANAGER & AUDITOR VIEW SIGNATURE DURING APPROVAL ---
# (Integrated into /pending route below)

# --- FEATURE 12, 13, 14 & SUB-INSTRUCTIONS 2 & 3: STATEMENT & RECEIPT FORMATTING ---
...
def print_receipt(txn_id):
    ...
    # SUB-INSTRUCTION 2: Commission muramu nagahee irratti akka hin mullane godhi
    return f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <div class="row"><span>Maqaa Nama Erguu:</span><b>{sender_info}</b></div>
        <div class="row"><span>Maqaa Nama Fudhatuu:</span><b>{receiver_info}</b></div>
...
# --- SUB-INSTRUCTION 3: STATEMENTA BALANCE SIRNAAN BARREESAA AKKA DEMUU GODHI ---
...
def statement(cust_id):
    ...
    for t in txns:
        ...
        # Omitted commission display per Sub-instruction 2
        rows_html = f"""
        <tr style="border-bottom:1px solid #e2e8f0; font-size:11px;">
            <td style="padding:8px;">{t['timestamp']}</td>
            <td style="padding:8px; font-weight:bold;">{t['ft_reference']}</td>
...
def pending():
    ...
    cards_html = ""
    if pending_custs:
        cards_html += "<h3 style='font-size:12px; color:#1e40af; margin-bottom:8px;'>👤 Galmee Maammiltoota Eeggamaa</h3>"
    ...
    if pending_txns:
        cards_html += "<h3 style='font-size:12px; color:#b45309; margin-top:16px; margin-bottom:8px;'>💵 Kaffaltii Mirkaneessa Eeggatu (Signature View)</h3>"
        for r in pending_txns:
            cards_html += f"""
            <div class="item-card">
                <div style="font-size:13px; font-weight:bold;">{r['txn_type']}: {r['amount']:,.2f} Birr - {r['customer_name']}</div>
                <div class="img-grid" style="margin-top:6px;">
                    <div style="text-align:center;"><img src="/uploads/{r['photo_path']}"><span style="font-size:10px;">Fuula</span></div>
                    <div style="text-align:center;"><img src="/uploads/{r['signature_path']}"><span style="font-size:10px; color:#c2410c; font-weight:bold;">Mallattoo ✍️</span></div>
                </div>
                <div style="text-align:right; margin-top:8px;">
                    <a href="/approve_txn/{r['txn_id']}" class="btn-action btn-green">✅ Approve Txn</a>
    ...
    content = f"""
    <div class="box">
        <h2 style="font-size:16px; color:#065f46; margin-bottom:12px;">📋 Manager & Auditor Approvals</h2>
        {cards_html if cards_html else '<p style="text-align:center; padding:20px; color:#64748b; font-size:12px;">Wanti mirkanaa\'u hin jiru.</p>'}
    </div>
    """
    return render_template_string(HTML_LAYOUT.replace("{% block content %}{% endblock %}", content), notifications=NOTIFICATIONS)
...
def approve_txn(txn_id):
    ...
    if txn:
        ...
        if t_type == 'DEPOSIT':
            ...
            cursor.execute("UPDATE customers SET balance = balance - ? WHERE customer_id = ?", (amt, c_id))
            cursor.execute("UPDATE customers SET balance = balance + ? WHERE customer_id = ?", (amt, t_acc))
...
def ceo_backup():
    ...
    return """
    <div class="box">
        <h2 style="font-size:16px; color:#581c87; margin-bottom:8px;">💾 Database Backup & Restore</h2>
        <p style="font-size:12px; color:#64748b;">Systemni ofuma isaatiin yeroo cufamuu fi banamu auto-backup fi restore godha.</p>
    </div>
    """
